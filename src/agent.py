...
#
#  torch.load uses weights_only=True to suppress the FutureWarning in
#  PyTorch ≥2.0 and avoid arbitrary code execution risk when loading
...
#
#  NOTE (design limitation, not a bug): this fixes a finite candidate
#  set of portfolios once, at construction time, seeded by SEED so it
#  is reproduced identically between train.py and evaluate.py. The
#  agent can only ever select among these candidates — it is not
#  learning a continuous weight vector. See config.py's ACTION SPACE
#  section for the size breakdown.
# ──────────────────────────────────────────────────────────────────────
...
class DQNAgent:
    ...
    def load(self, path: str):
        # weights_only=True suppresses FutureWarning in PyTorch ≥2.0
