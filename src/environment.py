...
#
#  NOTE: this environment is split-agnostic — it is instantiated
#  identically on the train / validation / test splits produced by
#  data.load_split_data(). All train/val/test separation logic lives
#  in train.py, evaluate.py, and main.py, not here.
# ============================================================
import numpy as np
from config import (
...
class PortfolioEnv:
    ...
    DQN-R reward is multiplied by RETURN_REWARD_SCALE (50.0) to bring
    gradient magnitudes in line with DQN-S. This is a monotone
    transformation and does not change the optimal policy — the
    ranking of actions by Q-value is unaffected.

    Transaction costs (TRANSACTION_FEE, turnover-based) and the
    concentration penalty (CONCENTRATION_PENALTY, HHI-based) are both
    applied here, to both DQN-Sharpe and DQN-Return, to discourage
    large/unstable reallocations.
    """
    ...
    def step(self, action: np.ndarray):
        ...
        # in portfolio weights vs. the prior step.
    ...
    def _compute_reward(self, daily_return: float,
        ...
        DQN-R: scaled daily portfolio return.
            Multiplied by RETURN_REWARD_SCALE (50.0) to match the
        ...
        # Applies to both reward types (previously only gated to
        # "return", which meant enabling it had no effect on
        # DQN-Sharpe — the agent whose training returns were actually
        # unstable).
